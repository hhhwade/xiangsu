...
## 已生成：Release 签名 APK v1.3.0

下载文件：

```text
travel-planner/release/xingji-smart-travel-amap-v1.3.0-release.apk
```
...
1. 若高德 POI 返回 `Photo`，显示该 POI 的真实图片；
2. 内置经典路线仅使用已核对的 Wikimedia Commons 实拍图；
3. 未能核对到该景点实拍图时，显示“暂无该景点实拍图”，**不使用泛化图片或 AI 图片冒充对应景点**。

内置实拍图来源清单：`travel-planner/native-amap/web/images/landmarks/CREDITS.md`。
...
发布版安全码 SHA1: F6:D0:77:4F:D2:A6:C7:BC:F7:EC:70:A2:56:1E:5E:5E:28:AF:10:80
```
...
| 景点概述与图片 | 每个景点展示概述；点击后显示高德实拍图、已核对实拍图或明确的无图状态 |
